...
public class Mvp2TalkModel implements MessageListener, Mvp2Contract.TalkModel {
    ...
    public ClientMessage sendInformation(String message) {
        ...
        if (imageURL.equals("")) {
            clientMessage = new ClientMessage(false, uuid, now, getUsername(), message);
        } else {
            clientMessage = new ClientMessage(true, uuid, now, getUsername(), imageURL);
        }
    ...
    public void onMessageReceived(Message message) {
        if (message instanceof ServerSendMessage) {
            ...
            if (imageURL.equals("")) {
                ...
                iMvp0TalkPresenter.receiveMessage(new ClientMessage(true, serverSendMessage.getMessageId(), serverSendMessage.getTime(), serverSendMessage.getSenderUsername(), imageURL));
            }
